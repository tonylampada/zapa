...
class TestAdminUsersEndpoints:
    ...
    def test_list_users_with_search(self, private_client: TestClient, mock_db: Session, auth_headers):
    ...
    def test_get_user_success(self, private_client: TestClient, mock_db: Session, auth_headers):
        ...
        # Mock message counts
        mock_db.query.return_value.filter.return_value.scalar.side_effect = [10, 5]  # sent, received
    ...
    def test_create_user_already_exists(self, private_client: TestClient, mock_db: Session, auth_headers):
    ...
    def test_delete_admin_user_forbidden(self, private_client: TestClient, mock_db: Session, auth_headers):
    ...
    def test_get_user_conversations_success(self, private_client: TestClient, mock_db: Session, auth_headers):
        ...
        mock_query.filter.return_value = mock_query
        mock_query.count.return_value = 3
        mock_query.order_by.return_value.offset.return_value.limit.return_value.all.return_value = messages
        mock_db.query.return_value = mock_query
        
        response = private_client.get(
            "/api/v1/admin/users/1/conversations",
            headers=auth_headers
        )
    ...
    def test_clear_user_conversations_success(self, private_client: TestClient, mock_db: Session, auth_headers):

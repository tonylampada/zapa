...
class TestAdminSystemEndpoints:
    ...
    def test_get_system_health_all_healthy(self, mock_psutil, mock_httpx, private_client: TestClient, mock_db: Session, auth_headers):
    ...
    @patch('app.private.api.v1.admin.system.httpx.AsyncClient')
    @patch('app.private.api.v1.admin.system.psutil')
    def test_get_system_health_degraded(self, mock_psutil, mock_httpx, private_client: TestClient, mock_db: Session, auth_headers):
    ...
    @patch('app.private.api.v1.admin.system.psutil')
    def test_get_system_health_unhealthy(self, mock_psutil, private_client: TestClient, mock_db: Session, auth_headers):
    ...
    def test_get_system_stats_success(self, private_client: TestClient, mock_db: Session, auth_headers):
        """Test getting system statistics."""
        # Mock user counts
        mock_db.query.return_value.scalar.side_effect = [100, 85, 5000, 150]  # total users, active users, total messages, messages today
        ...
        mock_db.query.return_value.filter.return_value.group_by.return_value.all.return_value = provider_counts
    ...
    def test_export_system_data_start(self, private_client: TestClient, mock_db: Session, auth_headers):
    ...
    def test_export_system_data_invalid_dates(self, private_client: TestClient, mock_db: Session, auth_headers):
    ...
    @patch('app.private.api.v1.admin.system.export_jobs', {"test-export-id": {"status": "completed", "download_url": "/download/test", "error_message": None}})
    def test_get_export_status_success(self, private_client: TestClient, auth_headers):
        ...
        response = private_client.get(
            "/api/v1/admin/system/export/test-export-id",
            headers=auth_headers
        )
    ...
    def test_get_export_status_not_found(self, private_client: TestClient, auth_headers):
        ...
        response = private_client.get(
            "/api/v1/admin/system/export/non-existent",
            headers=auth_headers
        )
    ...
    def test_system_endpoints_unauthorized(self, private_client: TestClient):
        ...
        for endpoint, method in endpoints:
            if method == "get":
                ...
                response = private_client.post(endpoint, params={"start_date": "2024-01-01", "end_date": "2024-01-02"})

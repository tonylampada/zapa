...
class TestWebhookEndpoints:
    ...
    async def test_webhook_endpoint_message_received(self, test_client: AsyncClient, test_db):
        ...
        with patch(
            "app.services.webhook_handler.AgentService.process_message", new_callable=AsyncMock
        ) as mock_process:
            mock_process.return_value = None

            response = await test_client.post("/api/v1/webhooks/whatsapp", json=event_data)
        ...
        message = (
            ...
            .filter(Message.media_metadata.op("->>")("whatsapp_message_id") == "msg_123")
            .first()
        )
    ...
    async def test_webhook_error_handling(self, test_client: AsyncClient):
        ...
        with patch(
            "app.services.webhook_handler.WebhookHandlerService.handle_webhook"
        ) as mock_handle:

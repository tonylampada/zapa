...
class TestWebhookIntegration:
    ...
    async def test_full_webhook_flow(self, test_db: Session):
        ...
        message = (
            ...
            .filter(Message.media_metadata.op("->>")("whatsapp_message_id") == "msg_test_123")
            .first()
        )
        ...
        mock_process.assert_called_once_with(
            user_id=user.id, message_content="Hello, can you help me?", message_id=message.id
        )
    ...
    async def test_webhook_retry_on_failure(self, test_db: Session):
        ...
        message = (
            test_db.query(Message)
            .filter(Message.media_metadata.op("->>")("whatsapp_message_id") == "msg_retry_test")
            .first()
        )
        assert message is not None
    ...
    async def test_webhook_media_message(self, test_db: Session):
        ...
        message = (
            test_db.query(Message)
            .filter(Message.media_metadata.op("->>")("whatsapp_message_id") == "msg_media_123")
            .first()
        )
        assert message is not None

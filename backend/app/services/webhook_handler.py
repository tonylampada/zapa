...
class WebhookHandlerService:
    ...
    async def _handle_message_received(
        ...
        try:
            ...
            # Extract phone number from WhatsApp JID (format: +1234567890@s.whatsapp.net)
            from_phone = data.from_number.replace("@s.whatsapp.net", "")
            
            # Find or create user
            user = self.db.query(User).filter(User.phone_number == from_phone).first()
            if not user:
                # Create new user
                user = User(
                    phone_number=from_phone,
                    display_name=f"User {from_phone[-4:]}",  # Default display name
                    is_active=True
                )
                self.db.add(user)
                self.db.commit()
                self.db.refresh(user)
                logger.info(f"Created new user for phone: {from_phone}")
            ...
            # Trigger agent processing if text message
            if data.text:
                try:

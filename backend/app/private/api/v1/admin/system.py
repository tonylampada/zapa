...
async def get_system_stats(
    ...
    average_response_time_ms = float(avg_response_time * 1000) if avg_response_time else 250.0  # Default 250ms
    
    # Get LLM provider usage
    provider_counts = db.query(
        LLMConfig.provider,
        func.count(LLMConfig.id)
    ).filter(
        LLMConfig.is_active == True
    ).group_by(LLMConfig.provider).all()

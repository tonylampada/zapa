...
async def whatsapp_bridge_check() -> Dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            try:
                status_response = await client.get(f"{settings.WHATSAPP_BRIDGE_URL}/status")
                bridge_data = status_response.json() if status_response.status_code == 200 else None
            except Exception:
                bridge_data = None
        ...
        raise WhatsAppBridgeError(f"WhatsApp Bridge returned error: {e.response.status_code}")
    except Exception as e:
